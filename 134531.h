...
public:
    ...
    ~BaseStack() {
        for (int i = 0; i < cap; i++) delete (_stack + i);
    }
    ...
    void DoubleCapacity() {
        ...
        T *newStack = new T[cap];        // create newStack
        for (int i = 0 ; i < cap / 2; i++) {   // copy element to newStack
            newStack[i] = _stack[i];
        }
...
void ShootSpecial(int col, int W) {
    if (SpBullet.empty() || col < 0 || col >= W) return;
